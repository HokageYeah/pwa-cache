...
const memFs = import("mem-fs");
const editor = import("mem-fs-editor");
...
class SWFilePlugin {
  apply(compiler) {
    compiler.hooks.emit.tapAsync("SWFilePlugin", (compilation, callback) => {
      ...
      const assets = Object.keys(compilation.assets).map(
        (asset) => `${publicPath}${asset}`
      );
      const importScripts = assets.filter((asset) =>
        /^\/db|network\.[a-zA-Z0-9]+\.js$/.test(asset)
      );
